...
function analyze(url, html, finalUrl, loadMs, headers, hasRobots, hasSitemap) {
  ...
  // BUSINESS GROWTH / LEAD CAPTURE SIGNALS
  const hasHeroCTA = hasAboveFoldSignal(html, ['book','schedule','call','contact','order','quote','get started','shop now','consultation']);
  const hasLeadForm = formCount > 0 || /newsletter|subscribe|lead|quote form|contact form|get a quote|request info/i.test(html);
  const hasChatWidget = /intercom|drift|tawk|crisp|zendesk|gorgias|tidio|livechat|chatwoot|messenger|chat-widget|chatbot|ai chatbot/i.test(html);
  const hasDirectCheckout = /add to cart|checkout|cart|buy now|shop pay|paypal|klarna|afterpay|sezzle/i.test(text);
  const hasServiceMenu = hasAny(text, ['services','menu','packages','products','shop','flavors','gallery','portfolio','custom orders']);
  const hasFAQ = /faq|frequently asked|common questions|questions/i.test(text);
  const hasGuarantee = /guarantee|satisfaction|refund|secure checkout|safe checkout|licensed|insured|certified/i.test(text);
  const hasLeadMagnet = /free estimate|free quote|free consultation|download|checklist|guide|coupon|discount|offer/i.test(text);
  const hasTrafficTracking = hasAnalytics || hasPixel;
  const hasRetargetingPixel = hasPixel;
  const hasConversionTracking = /gtag\(['"]event|conversion|AW-\d+|purchase|begin_checkout|add_to_cart|fbq\(['"]track/i.test(html);
  ...
  // GROWTH / CONVERSION SCORE
  let growthScore = 100;
  if (!hasHeroCTA) growthScore -= 16;
  if (!hasCTA) growthScore -= 14;
  if (!hasBooking && !hasDirectCheckout) growthScore -= 14;
  if (!hasLeadForm && !hasContact) growthScore -= 12;
  if (!hasPhone) growthScore -= 10;
  if (!hasReviews) growthScore -= 10;
  if (!hasServiceMenu) growthScore -= 8;
  if (!hasPricing) growthScore -= 6;
  if (!hasChatWidget) growthScore -= 5;
  if (!hasFAQ) growthScore -= 5;
  if (!hasGuarantee) growthScore -= 4;
  if (!hasLeadMagnet) growthScore -= 4;
  if (!hasTrafficTracking) growthScore -= 8;
  if (!hasRetargetingPixel) growthScore -= 5;
  if (!hasConversionTracking) growthScore -= 6;
  growthScore = Math.max(10, Math.min(98, growthScore));

  let score = Math.round(auditScore * 0.32 + perfScore * 0.22 + localScore * 0.23 + growthScore * 0.23);
  ...
  const signals = {
    ...
    hasHeroCTA, hasLeadForm, hasChatWidget, hasDirectCheckout, hasServiceMenu,
    hasFAQ, hasGuarantee, hasLeadMagnet, hasTrafficTracking, hasRetargetingPixel, hasConversionTracking,
  ...
  return {
    url: finalUrl, score, auditScore, localScore, perfScore, growthScore, platform,
    pageTitle: pageTitle || new URL(finalUrl).hostname,
    signals,
    audit: runAudit(signals),
    local: runLocal(signals),
    growth: runGrowth(signals),
    subscription: buildSubscriptionPitch(signals),
...
function hasAboveFoldSignal(html, words) {
  const body = html.replace(/<script[\s\S]*?<\/script>/gi,' ').replace(/<style[\s\S]*?<\/style>/gi,' ');
  const firstChunk = body.slice(0, 12000).replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').toLowerCase();
  return words.some(w => firstChunk.includes(w));
}
...
function runGrowth(s) {
  const checks = [], issues = [], opportunities = [];
  const addCheck = (icon, title, ok, good, bad, warn = false) => {
    checks.push({ icon, title, desc: ok ? good : bad, status: ok ? 'ok' : warn ? 'warn' : 'missing' });
  };

  addCheck('🚦', 'Above-the-fold action path', s.hasHeroCTA, 'A main action appears near the top of the site.', 'No strong top-of-page action was detected. Visitors may not know what to do next.');
  addCheck('🧲', 'Lead capture form', s.hasLeadForm, 'A form or lead capture path was detected.', 'No lead capture form was detected. Visitors can leave without becoming prospects.');
  addCheck('💬', 'Chat or instant help', s.hasChatWidget, 'Chat or instant support appears available.', 'No chat or instant support widget was detected.', true);
  addCheck('🛒', 'Direct purchase / checkout path', s.hasDirectCheckout, 'A checkout or direct purchase path was detected.', 'No direct purchase path was detected.', true);
  addCheck('📋', 'Services / product clarity', s.hasServiceMenu, 'The site appears to explain services or products.', 'Services/products are not clearly detected. Visitors may not understand what they can buy.');
  addCheck('❓', 'FAQ / objection handling', s.hasFAQ, 'FAQ or question-based content was detected.', 'No FAQ section was detected. Common buyer questions may go unanswered.', true);
  addCheck('📊', 'Traffic tracking', s.hasTrafficTracking, 'Analytics or ad tracking was detected.', 'No analytics or ad tracking was detected. The business may not know where traffic/leads come from.');
  addCheck('🎯', 'Retargeting pixel', s.hasRetargetingPixel, 'A retargeting/ad pixel was detected.', 'No retargeting pixel was detected. Warm visitors may not be reached again.', true);
  addCheck('✅', 'Conversion tracking', s.hasConversionTracking, 'Conversion tracking signals were detected.', 'No conversion tracking was detected. Paid traffic may be harder to measure.', true);

  if (!s.hasHeroCTA) issues.push({ title:'Add a strong above-the-fold action path', desc:'The first screen should quickly guide visitors to call, book, shop, or request a quote.', severity:'critical', category:'conversion' });
  if (!s.hasLeadForm && !s.hasContact) issues.push({ title:'Add lead capture', desc:'Add a form, quote request, newsletter, or chatbot so visitors can become leads before leaving.', severity:'critical', category:'lead_capture' });
  if (!s.hasBooking && !s.hasDirectCheckout) issues.push({ title:'Create a direct traffic-to-sale path', desc:'Visitors from Google, Instagram, ads, or referrals need a clear path to buy, book, or request service without extra steps.', severity:'critical', category:'direct_traffic' });
  if (!s.hasPhone) issues.push({ title:'Add mobile click-to-call', desc:'Mobile visitors often prefer to call quickly. A missing phone number can turn ready buyers into lost leads.', severity:'critical', category:'lead_capture' });
  if (!s.hasReviews) issues.push({ title:'Add visible social proof', desc:'Reviews, testimonials, ratings, and customer photos help visitors trust the business before spending money.', severity:'warning', category:'trust' });
  if (!s.hasServiceMenu) issues.push({ title:'Clarify what visitors can buy or book', desc:'Show services, packages, products, menu items, or custom order options clearly.', severity:'warning', category:'offer' });
  if (!s.hasTrafficTracking) issues.push({ title:'Add traffic tracking', desc:'Without analytics, the business cannot see which traffic sources are bringing visitors, leads, or sales.', severity:'warning', category:'traffic' });
  if (!s.hasRetargetingPixel) issues.push({ title:'Add retargeting capability', desc:'Many visitors do not buy the first time. Retargeting lets the business bring warm visitors back later.', severity:'info', category:'traffic' });
  if (!s.hasConversionTracking) issues.push({ title:'Track conversions, not just visits', desc:'A business needs to know which clicks turn into calls, bookings, purchases, or quote requests.', severity:'info', category:'measurement' });

  if (!s.hasCTA || !s.hasHeroCTA) opportunities.push('Turn more direct traffic into leads by adding stronger call-to-action buttons near the top of the site.');
  if (!s.hasPhone || !s.hasLeadForm) opportunities.push('Capture more mobile visitors with click-to-call and a simple lead form.');
  if (!s.hasReviews) opportunities.push('Increase buyer confidence by placing reviews, testimonials, or customer photos near booking/buy buttons.');
  if (!s.hasTrafficTracking || !s.hasConversionTracking) opportunities.push('Show business owners what traffic is working by adding analytics and conversion tracking.');
  if (!s.hasRetargetingPixel) opportunities.push('Bring back visitors who did not buy the first time using retargeting-ready tracking.');

  const criticalCount = issues.filter(i => i.severity === 'critical').length;
  const warningCount = issues.filter(i => i.severity === 'warning').length;
  const risk = criticalCount >= 3 ? 'High' : criticalCount >= 1 ? 'Medium' : 'Low';
  ...
  return {
    ...
    opportunities,
    impact: {
      trafficVisibilityRisk: (!s.hasLocalSchema || !s.hasTrafficTracking) ? 'High' : 'Moderate',
      leadCaptureRisk: (!s.hasPhone && !s.hasLeadForm) ? 'High' : (!s.hasPhone || !s.hasLeadForm) ? 'Medium' : 'Low',
      conversionRisk: risk,
      missedOpportunityCount: issues.length,
      criticalGrowthGaps: criticalCount,
      warningGrowthGaps: warningCount
    }
  };
}

function buildSubscriptionPitch(s) {
  const fixes = [];
  if (!s.hasLocalSchema) fixes.push('LocalBusiness schema');
  if (!s.hasPhone) fixes.push('click-to-call setup');
  if (!s.hasLeadForm) fixes.push('lead capture form');
  if (!s.hasHeroCTA || !s.hasCTA) fixes.push('stronger CTA placement');
  if (!s.hasReviews) fixes.push('review/social proof section');
  if (!s.hasTrafficTracking) fixes.push('traffic monitoring');
  if (!s.hasConversionTracking) fixes.push('conversion tracking');
  if (!s.hasRetargetingPixel) fixes.push('retargeting readiness');

  return {
    planName: 'Solviah Growth Monitoring',
    startingPrice: '$49/mo',
    agencyComparison: '$1,500+ one-time or monthly SEO quote vs. a smaller ongoing subscription',
    valuePosition: 'The scanner finds the gaps that can block traffic, leads, bookings, and sales, then positions Solviah as the ongoing system that monitors and improves them.',
    directTrafficMessage: 'When visitors come from Google, Instagram, ads, referrals, or direct links, the site should make it easy to understand the offer, trust the business, and book or buy without friction.',
    recommendedFixes: fixes,
    cta: 'Start monitoring and improving this website for $49/month.'
  };
}
